import { z } from 'zod';
import { Err, Ok, Result, SchemaError } from './errors';
  ...
  for (let i = 0; i < tiers.length; i++) {
    if (i > 0) {
      ...
      if (!firstTier || !secondTier) {
        return Err(new SchemaError({ message: 'No tier found' }));
      }

      const currentFirstUnit = firstTier.firstUnit;
      const previousLastUnit = secondTier.lastUnit;
