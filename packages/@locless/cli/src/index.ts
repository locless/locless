...
import { removeKey } from './commands/removeKey';
import { reset } from './commands/reset';
import { showProjects } from './commands/showProjects';
...
async function main() {
  ...
  if (
    ...
    console.error(
      chalk.gray('After installing `nvm`, install the latest version of Node with ' + chalk.bold('`nvm install node`.'))
    );
    console.error(chalk.gray('Then, activate the installed version in your terminal with ' + chalk.bold('`nvm use`.')));
    process.exit(1);
  }
