...
export const invoicingTask = schedules.task({
  ...
  run: async () => {
    ...
    workspaces = workspaces.filter(ws => ws.subscriptions && Object.keys(ws.subscriptions).length > 0);

    logger.info(`found ${workspaces.length} workspaces`);

    /**
     * Get the previous month's number, ie: if it's December now, it returns -> 11
     */
    const t = new Date();
