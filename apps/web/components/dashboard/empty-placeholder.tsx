...
EmptyPlaceholder.Icon = function EmptyPlaceHolderIcon({ children }: { children: React.ReactNode }) {
    return (
        <div className='flex items-center justify-center w-20 h-20 rounded-full bg-background-subtle'>{children}</div>
    );
};
...
EmptyPlaceholder.Description = function EmptyPlaceholderDescription({
    ...
    return (
        <p
            className={cn('text-content-subtle mb-8 mt-2 text-center text-sm font-normal leading-6', className)}
            {...props}
        />
    );
};
