...
export default function EditorPage(props: Props) {
  ...
  const handleChangeTree = (
    items: TreeItems<ElementNode>,
    reason: ItemChangedReason<ElementNode>
  ) => {
    ...
    if (activeItem && frameItem) {
      ...
      if (value === undefined) {
        ...
        newStyles = {
          ...newStyles,
          [styleTag]: {
            ...(newStyles[styleTag] ?? { name: styleTag, type: "custom" }),
            value,
          },
        };
      }
    ...
    if (activeItem && frameItem) {
      ...
      if (value === undefined) {
        ...
        newProps = {
          ...newProps,
          [propTag]: {
            ...(newProps[propTag] ?? { name: propTag, type: "custom" }),
            value,
          },
        };
      }
            ...
            {deviceFrames.map((device) => {
              return (
                <Button
                  key={device.title}
                  disabled={
                    device.title === localStorage.getItem("frameItemTitle")
                  }
          ...
          <TabsContent value="styles">
            {activeItem ? (
              <>
                ...
                {Object.entries(componentsData.styles[activeItem] ?? []).map(
                  (style) => (
                    <div
                      key={style[0]}
              ...
              <p className="text-black text-center">
                ...
                <br /> You can select any element simply by clicking on it in
                components tree.
              </p>
            )}
          </TabsContent>
          <TabsContent value="interactions">
            {activeItem ? (
              <>
                ...
                {Object.entries(componentsData.props[activeItem] ?? []).map(
                  (prop) => (
                    <div key={prop[0]} className="flex gap-4 items-center mb-4">
                      <Label
                        htmlFor={`props_${prop[0]}`}
                        className="text-black"
                      >
                        {prop[0]}
                      </Label>
