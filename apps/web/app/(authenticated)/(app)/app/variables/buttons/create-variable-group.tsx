...
export const CreateVariableGroupButton = ({ workspaceId, ...rest }: Props) => {
    ...
    const createVariableGroup = useMutation(api.variables.createGroup);
    ...
    const onSubmit = async ({ name }: z.infer<typeof formSchema>) => {
        ...
        try {
            ...
            toast({
                description: 'Your variable group has been created!',
            });
        } catch (err: any) {
            console.error(err);
            toast({
