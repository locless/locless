...
export default async function BillingPage() {
  ...
  return (
    <div className='flex flex-col gap-8 lg:flex-row p-4 lg:p-8'>
      <div className='w-full'>
        {workspace.plan === 'free' ? <FreeUsage workspace={workspace} /> : <PaidUsage workspace={workspace} />}
      </div>
    ...
    <Card>
      ...
      <CardContent className='flex flex-col gap-8 md:flex-row'>
        <ol className='flex flex-col w-2/3 space-y-6'>
          <MeteredLineItem
            title='Api Requests'
            tiers={[{ firstUnit: 1, lastUnit: QUOTA.free.maxApiRequests, centsPerUnit: null }]}
            used={usedApiRequests}
          />
          <MeteredLineItem
            title='Storage Usage'
            tiers={[{ firstUnit: 1, lastUnit: QUOTA.free.maxFileBandwidth, centsPerUnit: null }]}
            used={parseFloat((usedStorageUsage / 1048576).toFixed(5))}
          />
          <MeteredLineItem
            title='Storage Size'
            tiers={[{ firstUnit: 1, lastUnit: QUOTA.free.maxFileStorage, centsPerUnit: null }]}
            used={parseFloat((workspace.size / 1048576).toFixed(5))}
          />
        </ol>
        <div className='w-1/3'>
      ...
      <div className='flex flex-col items-center justify-center gap-8 md:flex-row lg:flex-col'>
        <div className='flex flex-col w-full gap-8'>
          ...
          <div className='flex items-center gap-8'>
            <Link href='/app/billing/stripe' className='w-full'>
...
const PaidUsage: React.FC<{ workspace: Workspace }> = async ({ workspace }) => {
  ...
  if (workspace.subscriptions?.apiRequests) {
    const cost = calculateTieredPrices(workspace.subscriptions.apiRequests.tiers, usedApiRequests);
    if (cost.err) {
  ...
  if (workspace.subscriptions?.fileBandwidth) {
    const cost = calculateTieredPrices(
      workspace.subscriptions.fileBandwidth.tiers,
      parseFloat((usedStorageUsage / 1048576).toFixed(5))
    );
    if (cost.err) {
      return <div className='text-red-500'>{cost.err.message}</div>;
    }
  ...
  if (workspace.subscriptions?.fileStorage) {
    const cost = calculateTieredPrices(
      workspace.subscriptions.fileStorage.tiers,
      parseFloat((workspace.size / 1048576).toFixed(5))
    );
    if (cost.err) {
      return <div className='text-red-500'>{cost.err.message}</div>;
    }
    ...
    <Card>
      ...
      <CardContent>
        <ol className='flex flex-col space-y-6'>
          ...
          {workspace.subscriptions?.apiRequests ? (
            <MeteredLineItem
              displayPrice
              title='Api Requests'
              tiers={workspace.subscriptions.apiRequests.tiers}
              used={usedApiRequests}
              forecast
            />
          ) : null}
          {workspace.subscriptions?.fileBandwidth ? (
            <MeteredLineItem
              displayPrice
              title='File Bandwidth'
              tiers={workspace.subscriptions.fileBandwidth.tiers}
              used={parseFloat((usedStorageUsage / 1048576).toFixed(5))}
              forecast
            />
          ) : null}
          {workspace.subscriptions?.fileStorage ? (
            <MeteredLineItem
              displayPrice
              title='File Storage'
              tiers={workspace.subscriptions.fileStorage.tiers}
              used={parseFloat((workspace.size / 1048576).toFixed(5))}
              forecast
            />
          ) : null}
      ...
      <CardFooter className='flex flex-col gap-4'>
        <div className='flex items-center justify-between w-full'>
          ...
          <span className='text-sm font-semibold tabular-nums text-content'>{formatCentsToDollar(currentPrice)}</span>
        </div>
        <div className='flex items-center justify-between w-full'>
          <span className='text-xs text-content-subtle'>Estimated by end of month</span>
          <span className='text-xs tabular-nums text-content-subtle'>{formatCentsToDollar(estimatedTotalPrice)}</span>
        </div>
      </CardFooter>
    </Card>
  );
                  ...
                  {tier.centsPerUnit ? (
                    <div className='flex flex-wrap items-baseline justify-between px-4 py-2 gap-x-4 gap-y-2 sm:px-6 xl:px-8'>
                      <dt className='text-sm font-medium leading-6 text-content-subtle'>
                        {' '}
                        {tier.centsPerUnit ? formatCentsToDollar(Number.parseFloat(tier.centsPerUnit), 4) : 'free'} per
                        unit
                      </dt>
