...
export default async function webhookHandler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const wh = new Webhook(webhookEnv()!.OPEN_METER_WEBHOOK_SECRET_ACTIVE_COMPONENTS_TOTAL);
...
const getPriceIdForMeter = (meterSlug: string, tier: 'hobby' | 'pro' | 'free' | 'enterprise') => {
  const { STRIPE_PRICE_COMPONENTS_HOBBY, STRIPE_PRICE_COMPONENTS_PRO } = stripeEnv()!;

  if (tier === 'enterprise' || tier === 'free') {
    return '';
  }

  if (meterSlug === 'active_components_total_webhook') {
    return `${tier === 'hobby' ? STRIPE_PRICE_COMPONENTS_HOBBY : STRIPE_PRICE_COMPONENTS_PRO}`;
  }
};
